...
import com.gym_membership.ai.common.util.BmiCalculator;
...
public class WorkoutPromptBuilderImpl implements WorkoutPromptBuilder {
	...
	public String buildPrompt(Member member, WorkoutRequest request) {
		...
		double bmi = BmiCalculator.calculate(member.getHeight(), member.getWeight());
		...
		prompt.append("\nBMI: ").append(String.format("%.1f", bmi));

		prompt.append("\nBMI Category: ").append(bmiCategory);
		prompt.append("\nWorkout Duration: ");

		if (request.getWorkoutDuration() != null) {
			prompt.append(request.getWorkoutDuration()).append(" minutes");
		} else {
			prompt.append("60 minutes (default)");
		}
